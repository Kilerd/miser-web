...
export function accountTreeGenerator(
  value: { [id: number]: Account },
  listIncludeClosedAccount: boolean
) {
  ...
  Object.values(value)
    ...
    .forEach((it) => {
      ...
      if (targetCategory.children[leafItem] === undefined) {
        ...
        targetCategory.children[leafItem].amount = targetCategory.children[
          leafItem
        ].amount.plus(it.summary.total.value);
        targetCategory.children[leafItem].id = it.id;
      }

      parents.forEach((one_parent) => {
        one_parent.amount = one_parent.amount.plus(
          new Big(it.summary.total.value)
        );
      });
    });
  return ret;
...
function Accounts() {
  ...
  const [listIncludeClosedAccount, setListIncludeClosedAccount] =
    useState(false);

  const accountTreeGenerator1 = accountTreeGenerator(
    ledgerContext.accounts,
    listIncludeClosedAccount
  );
  return (
    <>
      <AuthenticationLayout>
        <div className="container">
          <h1>Accounts</h1>
          <button onClick={() => ledgerContext.update("ACCOUNT")}>
            refresh
          </button>
          <Switch
            checked={listIncludeClosedAccount}
            label="include Closed Account"
            onChange={() =>
              setListIncludeClosedAccount(!listIncludeClosedAccount)
            }
          />
          ...
          <EditAccountModal
            {...editAccountData}
            modalStatus={editAccountModalStatus}
