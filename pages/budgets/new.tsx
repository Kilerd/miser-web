import {Button, Checkbox, FormGroup, H1, InputGroup, Radio, RadioGroup} from "@blueprintjs/core";
...
function Page() {
    ...
    const submit = async () => {
        const targetAccounts = Object.keys(accounts).filter(it => accounts[it]).map(it=> parseInt(it, 10));
        await api.createBudget(name, description, periodic, amount, currency, targetAccounts)
            ...
            <div className="container">
                ...
                <div>
                    ...
                    <FormGroup label="amount" labelFor="text-amount">
                        <InputGroup id="text-amount" placeholder="amount" value={amount}
                                    onChange={e => setAmount(e.target.value)}/>
                    </FormGroup>
                    <FormGroup label="currency" labelFor="text-currency">
                        <InputGroup id="text-currency" placeholder="currency" value={currency}
                                    onChange={e => setCurrency(e.target.value)}/>
                    </FormGroup>
                    <div>
                        {Object.keys(accounts).map(it =>
                            <Checkbox checked={accounts[it]} label={ledgerContext.getAccountAlias(parseInt(it, 10))}
