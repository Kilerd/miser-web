...
const Client = (props: Props) => {
  ...
  const submit = async () => {
    setLoading(true);
    const selectedCommodityMap = Object.keys(editCommodityMap).filter(
      (one) => editCommodityMap[one]
    );
  ...
  const updateStatus = async () => {
    await api.updateAccountStatus(
      props.id,
      props.status === "Open" ? "Close" : "Open"
    );
    ledgerContext.update("ACCOUNT");
  };
      ...
      <div>
        {Object.keys(editCommodityMap).map((commodity) => (
          ...
          >
            <input
              type="checkbox"
              checked={editCommodityMap[commodity]}
              onChange={() => handleCommodityClick(commodity)}
              className="input"
