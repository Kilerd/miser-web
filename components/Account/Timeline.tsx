...
const Client = (props: Props) => {
  ...
  const {
    ...
    getUrlByTime(
      `/ledgers/${ledgerContext.ledger_id}/accounts/${props.id}/timeline`,
      "create_time"
    ),
    get
  );
              ...
              if (item.type === "Transaction") {
                return (
                  <tr key={`${item.type}-${item.transaction_id}`}>
                    ...
                    <td style={{ textAlign: "right" }}>
                      <Amount
                        amount={item.balance_number}
                        commodity={item.balance_currency}
                      />
                    </td>
                  </tr>
                );
              ...
              if (item.pad !== null) {
                ...
                return (
                  <tr key={`${item.type}-${item.transaction_id}`}>
                    ...
                    <td>
                      {item.pad === accountId
                        ? `change by account ${ledgerContext.getAccountAlias(
                            item.account
                          )} balance`
                        : ""}
                    </td>
                    <td style={{ textAlign: "right" }}>
                      {item.pad === accountId ? (
                        <Amount
                          amount={item.detail.change[0]}
                          commodity={item.detail.change[1]}
                          color
                        />
                      ) : (
                        <Amount
                    ...
                    <td style={{ textAlign: "right" }}>
                      <Amount amount={item.amount} commodity={item.commodity} />
                    </td>
                  </tr>
                );
              }
                    ...
                    {item.detail?.balance ? (
                      ...
                      <div>
                        ...
                        <div>
                          current:{" "}
                          <Amount
                            amount={item.detail.current[0]}
                            commodity={item.detail.current[1]}
                            color
                          />
                        </div>
                        <div>
                          distance:{" "}
                          <Amount
                            amount={item.detail.distance[0]}
                            commodity={item.detail.distance[1]}
                            color
                          />
                        </div>
                      </div>
