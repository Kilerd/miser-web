...
export default function NewAccountModal({ modalStatus, setModalStatus }) {
  ...
  const [account, setAccount] = useState(() => {
    const account1 = Object.values(ledgerContext.accounts).find(
      (it) => it.full_name === "Equity:Opening-Balances"
    );
    return account1?.id;
  });
  ...
  function handleAccountChange(e: React.ChangeEvent<HTMLSelectElement>) {
    ...
    let commodityCandidates =
      ledgerContext.accounts[accountId]?.commodities || [];
  ...
  const submit = async () => {
    setLoading(true);
    const selectedCommodityMap = Object.keys(commodityMap).filter(
      (one) => commodityMap[one]
    );
    await api.newAccount(
      name,
      alias,
      selectedCommodityMap,
      init,
      account,
      amount,
      commodity
      ...
      <div>
        {Object.keys(commodityMap).map((commodity) => (
          ...
          >
            <input
              type="checkbox"
              checked={commodityMap[commodity]}
              className="input"
          ...
          <div>
            ...
            >
              {commodityCandidates.map((candidate) => (
                <option selected={commodity === candidate} value={candidate}>
                  {candidate}
                </option>
              ))}
            </select>
